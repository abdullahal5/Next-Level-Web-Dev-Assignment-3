...
export const initiatePayment = async (payload: TransactionData) => {
  const response = await axios.post(config.PAYMENT_URL!, {
    ...
    success_url: `${config.Backend_URL}/api/v1/payment/confirmation?transactionId=${payload.transactionId}&status=success&payload=${encodeURIComponent(JSON.stringify(payload))}&userId=${payload.userId}`,
